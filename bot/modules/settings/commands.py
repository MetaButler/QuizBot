...
async def settings(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if chat_member.status not in (ChatMember.ADMINISTRATOR, ChatMember.OWNER):
        await message.reply_text(
            text="Only administrators and owners can change group settings for quizzes",
            reply_to_message_id=message.id,
    ...
    if group_settings is None:
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [InlineKeyboardButton('Quiz Repeat: 1H', callback_data=f'grp_rpt_{chat.id}_{user.id}')],
                [InlineKeyboardButton('Poll Timeout: Off', callback_data=f'grp_tmt_{chat.id}_{user.id}')],
                [InlineKeyboardButton('Select Categories', callback_data=f'grp_cat_{chat.id}_{user.id}')],
                [InlineKeyboardButton('Reset Quiz State', callback_data=f'grp_rst_{chat.id}_{user.id}')],
                [InlineKeyboardButton('Close Settings', callback_data=f'grp_cnl_{chat.id}_{user.id}')],
            ]
        )
    else:
        timeout_mapping = {
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [InlineKeyboardButton(f'Quiz Repeat: {timeout_mapping.get(group_settings.get("timer"))}', callback_data=f'grp_rpt_{chat.id}_{user.id}')],
                [InlineKeyboardButton(f'Poll Timeout: {poll_timeout_mapping.get(group_settings.get("poll_timeout"))}', callback_data=f'grp_tmt_{chat.id}_{user.id}')],
                [InlineKeyboardButton('Select Categories', callback_data=f'grp_cat_{chat.id}_{user.id}')],
                [InlineKeyboardButton('Reset Quiz State', callback_data=f'grp_rst_{chat.id}_{user.id}')],
                [InlineKeyboardButton('Close Settings', callback_data=f'grp_cnl_{chat.id}_{user.id}')],
            ]
