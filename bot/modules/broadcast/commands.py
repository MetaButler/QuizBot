...
async def broadcast(update: Update, _: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for i in range(len(buttonurl_matches)):
        button_urls.append(str(buttonurl_matches[i][-1]).encode('raw_unicode_escape').decode('unicode_escape'))
        button_texts.append(str(buttonurl_matches[i][0]).encode('raw_unicode_escape').decode('unicode_escape'))
        button_urls[i] = str(button_urls[i]).replace('\\', '')
        if str(button_urls[i]).startswith("{{") and str(button_urls[i]).replace("{{", "").replace("}}", "") in broadcast_config.keys():
            button_urls[i] = broadcast_config[str(button_urls[i]).replace("{{", "").replace("}}", "")]
    ...
    await message.reply_text(
        text=f"This is how your broadcast will look like:\n\n{message_as_markdown.strip()}",
        reply_to_message_id=message.id,
        allow_sending_without_reply=True,
        quote=True,
    ...
    await _.bot.send_message(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton('Yes ✅', callback_data=rf'brdcst_yes_{user.id}'),
